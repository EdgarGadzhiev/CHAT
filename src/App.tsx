...
function TopBar({ displayName, onProfile }: { displayName: string; onProfile: () => void }) {
  return <header className="topbar">
    ...
    <button className="icon-button glass-button" aria-label="Уведомления"><Bell size={19} /><span className="notification-dot" /></button>
  </header>
}
...
function DatingPage({ balance, onSpend }: { balance: number; onSpend: () => void }) {
  ...
  return <section>
    ...
    <div className="dating-card"><div className="dating-photo-placeholder"><Heart size={34} /><span>Фото появятся здесь</span></div><div className="dating-profile"><span className="status-pill"><i /> Сейчас онлайн</span><h2>Случайная встреча</h2><p>Познакомься с новым человеком из своего города.</p></div><div className="dating-actions"><button className="round-action secondary"><X size={22} /></button><button className="main-dating-action" disabled={!canEnter || entered} onClick={() => { if (canEnter) { onSpend(); setEntered(true) } }}><Heart size={20} fill="currentColor" />{entered ? 'Ты в поиске' : 'Найти пару'}</button></div><div className="price-note">Вход для мужчин — <strong>10 NC</strong> · для девушек — бесплатно</div></div>
  </section>
}
...
function CityPage() {
  return <section>
    ...
    <div className="city-grid">{cityServices.map(({ icon: Icon, title, text, badge }) => <button className="city-card" key={title}><div className="city-card-icon"><Icon size={20} /></div><span className="city-badge">{badge}</span><strong>{title}</strong><p>{text}</p><ChevronRight size={16} /></button>)}</div>
...
function ProfilePage({ user, balance }: { user: TelegramUser | null; balance: number }) {
  ...
  return <section>
    ...
    <div className="profile-card"><div className="profile-avatar">{user?.photo_url ? <img src={user.photo_url} alt="" /> : initials}</div><div className="profile-name"><h2>{name}</h2><span>{user?.username ? `@${user.username}` : 'Telegram аккаунт'}</span></div><button className="icon-button profile-more"><MoreHorizontal size={18} /></button></div>
    ...
    <div className="settings-list"><button><span className="settings-icon"><Wallet size={18} /></span><div><strong>Nurcoin</strong><small>История операций и покупки</small></div><ChevronRight size={17} /></button><button><span className="settings-icon"><Bell size={18} /></span><div><strong>Уведомления</strong><small>Настройки уведомлений</small></div><ChevronRight size={17} /></button><button><span className="settings-icon"><CircleHelp size={18} /></span><div><strong>Помощь и правила</strong><small>Безопасность и поддержка</small></div><ChevronRight size={17} /></button></div>
...
function ChatRoom({ room, onBack }: { room: Room; onBack: () => void }) {
  ...
  return <section className="chat-screen">
    <header className="chat-header"><button className="back-button" onClick={onBack}>‹</button><div className="room-avatar room-avatar-small">{room.emoji}</div><div className="chat-heading"><strong>{room.name}</strong><span><i /> {room.online} онлайн</span></div><button className="icon-button"><MoreHorizontal size={19} /></button></header>
    <div className="chat-rules">Общайся уважительно · без спама · соблюдай правила</div>
    <div className="messages">{messages.map((item, index) => <div className={`message-row ${item.own ? 'message-own' : ''}`} key={`${item.time}-${index}`}><div className="message-avatar">{item.name[0]}</div><div className="message-bubble"><div className="message-meta"><strong>{item.name}</strong><span>{item.time}</span></div><p>{item.text}</p></div></div>)}</div>
